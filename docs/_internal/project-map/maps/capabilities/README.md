...
- Agent profiles can also carry `use` selectors; configured and dynamic
  ...
  work.
- Background task lifecycle is flat in v1. Child agents do not receive
  `task_create`; `capabilities.agents.maxDepth` bounds ordinary child/delegate
  spawning and does not enable nested background work.
...
- Read: host config/schema/runtime, CLI capability projection, agent-runtime
  child spawn options.
- Tests: focused P4 matrix, schema check, affected workspace builds, Terra
  foreground-only CLI runs.
