...
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| Main-agent max steps       | `run.maxSteps` + `run.budget.maxModelCalls`                                                                                        | config.ts:124        |
| Main run budget            | `run.budget.{maxDurationMs,maxModelCalls,maxToolCalls,maxTokens,maxCostUsd}`                                                       | config.ts:1659       |
...
| Write guardrails           | `policy.write.{maxFiles,maxDiffLines,allowDeletions}`                                                                              | config.ts:132        |
| Read confidentiality       | `policy.confidentialPaths`                                                                                                         | config.ts:105        |
| Shell sandbox              | `policy.sandbox.{mode,filesystem,network,failIfUnavailable}`                                                                       | config.ts:150        |
| Approval defaults          | `approvals.{shellSafe,edits,all}` + `permissionMode` (named modes)                                                                 | config.ts:141/366    |
| Verification gate          | `capabilities.verification.{mode,profiles,afterWrites}` (`stopGate` and `afterWrites.frequency` removed)                           | config.ts:240        |
| Workflow hooks             | `capabilities.hooks.workflow[]`                                                                                                    | config.ts:206        |
| Trace verbosity            | `run.traceLevel`                                                                                                                   | config.ts:127        |
...
**Must use the canonical grouped form `identity/policy/run/ui`.** The external
schema and runtime loader now reject the former root aliases; the historical
flat `SharedConfig` remains only as the post-validation carrier. Mapping:
`identity.{model,providers}`,
`policy.{confidentialDefaults,confidentialPaths,write,sandbox}`,
`run.{accessMode,backgroundTasks,budget,maxSteps,traceLevel}`, and
`ui.{theme,mouse,keybindings,vim}`. `workspace`, `shell.foregroundTimeoutMs`,
`tools`, `tasks`, and `capabilities` remain canonical top-level fields.
...
defer`, `policy.sandbox.*`, `policy.confidentialPaths`, `verification.*`,
`hooks.workflow`, `run.traceLevel`, `agents.profiles/delegateTools`, sub-agent
details.
...
(TUI-owned), `command_allowlist` (covered by policy.sandbox + approvals).
...
| **P3 (LAST)** | **Done.** P3a introduced `packages/host/src/config-zod-schema.ts` and `scripts/generate-config-schema.ts`, so `schemas/config.schema.json` is generated from the host-owned Zod schema and `npm run schema:check` enforces drift before Ajv validation. Subsequent slices moved host-owned types, primitive checks, allowed-key lists, enums, grouped sections, provider/model/cost validation, and workflow action branches onto that source. The later grouped-only consolidation removed root-alias tolerance, grouped-vs-flat conflicts, and the public group-to-flat map; runtime and JSON Schema now enforce the same canonical root vocabulary while the loader preserves partial recovery inside valid sections. **Independent follow-up:** `schemas/agent-profile.schema.json` remains hand-maintained because it is the separately owned Agent-profile schema referenced by config. Its duplicated selector enum is protected by `packages/cli/test/config-schema.test.ts` until Agent-profile generation is addressed as its own mechanism. | L    |
...
- `config-zod-schema.ts` factors shell sandbox filesystem/network schemas and
  ...
  the same Zod source.
- `config.ts` uses those key lists for unknown-field diagnostics in
  `policy.sandbox`, `policy.sandbox.filesystem`, `policy.sandbox.network`,
...
- `config-zod-schema.ts` exports key lists derived from Zod object schemas for
  ...
  and `capabilities.verification.afterWrites`.
...
- `config-zod-schema.ts` exports named schemas and key lists for the canonical
  grouped config sections `identity`, `policy`, `run`, and `ui`.
- The Host loader maps those groups into its internal carrier only after
  rejecting unknown roots. `policy.sandbox` is combined internally with the
  independently canonical `shell.foregroundTimeoutMs`; no external
  `shell.sandbox` or grouped-vs-flat conflict path remains.
...
- Scope: workflow-runtime-v1 D25 config surface: verification keeps
  `capabilities.verification.{mode,profiles,afterWrites}` while `stopGate` and
  `afterWrites.frequency` are intentionally removed from strict config/schema;
  `afterWrites.injectOutput` remains the require-mode retry evidence control.
...
"verification profiles|afterWrites frequency"`; `npm run schema:check`;
