...
Status: P0-P4 implemented and sealed; P4+ and P5 deferred behind explicit reopen conditions
Date: 2026-07-11
Branch context: `feat/background-task-governance-seal`
...
## 2026-07-11 Closure Audit

Source, focused tests, capability inspection, and real
`openai/gpt-5.6-terra` runs establish that P0-P4 are implemented. P4 governance
now has one effective `backgroundTasks` decision across run/config ceilings,
shell direct-background and promotion, `task_create`, dynamic `spawn_agent`,
resume authorization snapshots, trace metadata, and capability inspection.
`foreground-only` rejects explicit background shell execution, forces
`task_create(mode:"background")` inline, and prevents `spawn_agent` promotion.
The final diagnostic drift in CLI capability output was fixed by projecting the
runtime shell snapshot instead of a hard-coded promotion flag.

P4+ is not reopened. Current evidence proves only live in-process
`waiting_tasks`, durable terminal notification replay, and explicit orphan
failure on resume. There is no first-class suspended outcome, durable barrier
intent, crash-atomic notification/context/checkpoint transaction, or CLI
`--detach`; no user requirement in this audit needs awaited work to survive CLI
exit. Reopen only for a concrete detach/resume use case, beginning with the
suspended outcome and crash-consistency contract before implementation.

P5 is not reopened. The earlier `allowNestedBackgroundTasks` opt-in was a
premature partial implementation: it had a real-model QA canary but no user
case and did not close tree-wide budget/concurrency, durable lineage,
cancellation propagation, or TUI tree projection. The opt-in, nested child
`task_create` injection, and child revival registry were removed. V1 is flat:
...
implemented a _different subset_ of the same lifecycle:
...
At that baseline the revival half was unbuilt: `TaskManager` could emit notifications and a
...
## 2026-07-10 Revision — Explicit Background And Service Semantics

This revision is part of this proposal and supersedes the older shortcut that
implemented `background:true` as foreground promotion with a zero-millisecond
budget. It does not create a parallel lifecycle proposal.

Resolved decisions:

1. **Explicit background is not promotion.** A shell call with
   `background:true` passes normal validation, policy, and approval, then uses a
   direct background handoff. Its result carries `background:true` and
   `backgroundOrigin:"explicit"`; it does not carry `promoted:true` or claim
   that the foreground budget expired. Timeout promotion carries
   `backgroundOrigin:"promoted"` and retains `promoted:true`.
2. **Approval resolves before detach.** Direct background execution may not
   spawn or create task state until the synchronous core policy/approval gate
   has allowed the call. A denial returns on the current tool call and creates
   no process/task.
3. **Keep one awaited bit.** Explicit background shell tasks set
   `awaited:false`: their terminal notification is still delivered when a live
   run has another turn, but they do not keep the run alive. Foreground timeout
   promotion sets `awaited:true` because the original foreground caller still
   requires the result. No silent third result-policy state is added.
4. **Job/service is an orthogonal lifecycle dimension.** `lifetime:"job"`
   remains the default. `lifetime:"service"` means spawn success plus a short
   internal grace window without immediate exit is a successful launch. V1 has
   no port probe, output matcher, health protocol, or model-configurable grace
   duration. A later service exit still follows normal terminal task handling.
5. **Equivalent explicit shell work is deduplicated before spawn.** Within one
   parent run, active explicit shell tasks key on normalized command, canonical
   cwd, and lifetime. A duplicate returns the existing task id and does not
   start another process.
6. **Guidance stays sparse.** Background facts enter model context through the
   start/dedup tool result, terminal notification, and natural-terminal awaited
   check. There is no fixed every-turn task inventory.
7. **Cancellation owns terminality.** If cancellation wins while final model
...
9. **The handoff carries the resolved two-dimensional policy.** Shell-tool
   ...
   keep-alive from `origin`. This keeps `awaited` as the generic TaskRecord
   contract while leaving job/service startup semantics at the shell boundary,
   where they are meaningful. No generic task-lifetime field is introduced.

## Historical Baseline Facts (2026-07-01)

These facts explain the original design route; use the revision and active
project maps for current behavior:
...
- **Foreground** — the tool call _blocks the run loop_ (`await handle.wait()`).
...
Promoted and background converge onto the _same_ detached lifecycle; the only
...
| ---------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Auto**   | foreground budget timer             | budget overrun fires `onBackground` (shell's pattern), detach + ticket                                                                                  |
...
- The fg/bg decision is _partly deferred and automatic_, reducing the burden on
  the model.
...
orthogonal to (a).** `drainNotificationSources` only runs _at the start of a
step the loop was already going to take_. It cannot rescue an idle loop: if the
...
- On model completion, **before terminating**, check "are there **awaited**
  non-terminal tasks?" (§5). If yes, **block _inside_ `runLoop()`** on a
...
Rule: **the loop suspends in `waiting_tasks` only while ≥1 _awaited_ non-terminal
...
(Pausing the whole loop is a _third_, unrelated feature — out of scope here.)
...
- **join-any vs join-all barrier**. If the model opened N tasks and needs _all_
...
result/err summary` so the model can tell _which_ of many finished (and which
...
2. **Promote a running inline sub-agent to background** — _not wired; feasible
   ...
   **parent-visible contract**: `subagent.*` events, usage rollup into the
   ...
   it is _"detach execution, retain parent projection"_ — which is exactly why
   promotion should be **one shared primitive** (Consolidation §B), not
   reimplemented per surface.
3. **Let a sub-agent itself create background tasks (nesting)** — _keep gated_.
...
- **P0 — revival spine (five parts, all required).**
  ...
  4. **Suspend semantics**: `waiting_tasks` as an **internal block inside
     ...
     _awaited_ non-terminal tasks (§4b, §5). `commandReady` comes from
  ...
  5. **Awaited contract**: add first-class `TaskRecord.awaited` (plus store,
     ...
     _(Delivers: today's background tasks can correctly wake the agent — and be
     audited via one canonical event.)_
- **P1 — foreground task + promotion + general cap.** `task_create` gains
  foreground/`awaited` mode (`await handle.wait()`); foreground budget +
  `onBackground` returning a ticket ("default foreground, auto-promote"); complete
...
- **P3 — sub-agent promotion (projection-preserving).** Inline `spawn_agent`
  ...
  ledger attribution.
- **P4 — governance (complete).** Session-level fg/bg policy is resolved with
  the access-mode clamp and consumed by shell, `task_create`, `spawn_agent`,
  resume metadata, trace, and capability inspection (Consolidation §D).
...
- **P5 (deferred) — nesting.** V1 remains flat. Reopen only for a concrete
  nested background-agent use case and design the full lineage/authorization/
  budget/cancellation/TUI matrix before restoring an opt-in.
...
The point of this proposal is that these features are _one primitive seen from
three angles_. Concrete merges to enforce that, each cutting future maintenance:
...
- **§A — One notification contract, one home.** `NotificationSource` /
  ...
  contract to core (or a shared package) so **both** loops consume the _same_
...
- **§B — One promotion primitive.** shell, task, and sub-agent promotion are the
  same operation: _detach live execution, hand lifecycle to `TaskManager`,
  return a ticket, preserve the parent-visible projection (trace/usage/terminal)_.
...
- **§D — One `awaited` flag, three consumers.** The record's awaited/detached
  intent drives _loop-suspend_ (§4b), _CLI orphan policy_ (§9), and
  _barrier/revival accounting_ from a single field — not three ad-hoc
...
- Changing the shell promotion contract; this proposal _generalizes_ it.
