...
| TUI-local 配置字段                       | `packages/host/src/config-zod-schema.ts` + `packages/tui/src/lib/config.ts`                                                                                                                                                                                                                                                                                                                                                                                   | **历史方案已被实现后的 canonical contract 取代。** 文件配置只接受 `run.accessMode` 与 `ui.{theme,mouse,keybindings,vim}`；Host loader 统一验证/合并，TUI 不再拥有 flat 配置字段或第二套 normalization。                         |
