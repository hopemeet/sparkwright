...
- Host creates a connection principal from transport/auth configuration and the
  completed handshake. A request cannot supply a principal id, `system`, or a
...
Self-binding is disabled by default. An operator may explicitly enable it for a
trusted gateway connection; even then Host intersects requested permissions
