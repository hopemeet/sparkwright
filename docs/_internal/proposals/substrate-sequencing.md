...
Corollary for proposals: a proposal may _reference_ a Tier 1 substrate; it
...
1. **Work budget — not S3, unchanged.** `maxSteps` + `runBudget`
   (tokens / cost / model calls) bound how much _primary_ work a run may
...
| Interactive execution ownership / "next execution" boundary | session-agent-host-coordinator.md v4 ownership matrix                 | core owns in-run command consumption, Workflow owns Workflow/todo episode advancement, `TaskManager` owns Task terminal/revival facts, and future `ExecutionLaneCoordinator` owns only bounded interactive-lane queueing, idempotency, capacity, and active-execution handoff. Host decomposition precedes coordinator wiring; workspace mutation leases remain a separate Host service, and compatibility adapters must use the same canonical Host control path.     |
