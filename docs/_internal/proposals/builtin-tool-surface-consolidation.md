...
| agent | `create_agent` | agent 创建(`createAgentManagerTool`) |
...
- `web_search`:输入 `query,maxResults?,allowedDomains?,blockedDomains?`;只返回
  `{ title,url,snippet?,publishedAt?,source? }`,不抓正文,`maxResults` 有硬上限(建议
  10)。输出带 `query/provider/fetchedAt/results`。系统 prompt 要求模型引用这些 URL,
