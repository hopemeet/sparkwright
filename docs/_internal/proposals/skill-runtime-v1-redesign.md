...
> _internals_. This section is about _exposure_: how each external-capability
...
- **Invoking an Agent / MCP tool = a real call** with side effects, latency,
  ...
  is expensive. So an invoked capability must keep _discovery_ resident (the
  model must see the index without paying a search round-trip first) even while
  it defers _schema_ and folds individual capabilities behind a generic invoker.
...
- **Per-target guardrails preserved through the facade.** `delegate_agent`
  ...
  the key correctness property: the generic tool is a _router_, not a
  reimplementation.
